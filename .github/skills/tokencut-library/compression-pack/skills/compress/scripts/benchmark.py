...
def print_table(rows):
    ...
    print("|------|----------|------------|---------|-------|")
    for r in rows:
        print(f"| {r[0]} | {r[1]} | {r[2]} | {r[3]:.1f}% | {'âœ…' if r[4] else 'âŒ'} |")
...
def main():
    ...
    if len(sys.argv) == 3:
        ...
        if not orig.exists():
            print(f"âŒ Not found: {orig}")
            sys.exit(1)
        if not comp.exists():
            print(f"âŒ Not found: {comp}")
            sys.exit(1)
        print_table([benchmark_pair(orig, comp)])
        return

    # Glob mode: repo_root/tests/Tokencut-compress/
    tests_dir = Path(__file__).parent.parent.parent / "tests" / "Tokencut-compress"
    if not tests_dir.exists():
        print(f"âŒ Tests dir not found: {tests_dir}")
        sys.exit(1)
